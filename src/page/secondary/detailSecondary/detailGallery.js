...
export const DetailGallery = ({ title, cards, columns }) => {
    const columnStyles = {
        1: 'columns-1',
        2: 'columns-2',
        3: 'columns-3',
    ...
    return (
        <div className={"flex flex-col justify-center mb-32"}>
            ...
            <div className={`ml-12 mt-12 ${columnStyles[columns]}`}>
                {cards.map((card) => {
                    return (
                        <Link key={card.tokenId} to={`/secondary/${card.assetContractAddress}/${card.tokenId}/${card.id}`}>
                            <Cards key={card.tokenId} {...card} />
                        </Link>
                    )
                })}
            </div>
